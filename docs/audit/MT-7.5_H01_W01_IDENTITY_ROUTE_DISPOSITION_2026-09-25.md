...
| 04 | `GET\|HEAD admin/activity` | Protected GET /internal/admin/website-audit -> WebsiteAuditViewer, 50/page source-scoped redacted HTTP plus Admin identity domain events, q/method filters; identity events have no fabricated HTTP method/path | H01 focused PHP H01WebsiteAuditTest 1/1 (99 assertions) and real Edge 2/2 PASS; private/role/390px/CSRF-safe GET, synthetic cleanup; original Blade URL retired |
...
| 11 | `GET\|HEAD admin/dashboard` | Protected GET /internal/admin/website-performance plus /website-commerce and /digital-operations; combined read-only Website performance over single target commerce/digital order authority, bounded date/filters | H01 targeted PHP combined report 1/1 (43 assertions), real Edge 2/2 PASS with synthetic owner/data cleanup; separate net settlement/profit/verified-payment units not conflated |
| 12 | `GET\|HEAD admin/dashboard/report.csv` | GET /internal/admin/website-performance/orders.csv combined detailed 10-column, oldest-first formula-safe 5,000-order maximum with explicit error plus unchanged W03 commerce CSV | H01 focused combined commerce+digital three-order CSV and private Admin browser download PASS; bounded report/historical export retirement explicitly documented in W01 dashboard behavior review |
