...
- Explicit necessary CI request commit `7df4142a0909667cbc91e738669c81b5542fcba3` names exact source parent `89a5a66`, correct project ID and approved GITHUB execution mode. Clean hosted run `35628330989` has PASSED request validation, isolated MySQL preparation, Composer/PHP Pint, backend build, full backend regression, explicit MySQL race/reset, Website build, unchanged bundle budgets and secret scan. Its POS/Admin Playwright step was still running at this checkpoint. W01/P02/W03 family-specific browser steps were intentionally SKIPPED by the scoped request. Do not represent the entire run or W04 family as accepted unless terminal verification supports that claim.
