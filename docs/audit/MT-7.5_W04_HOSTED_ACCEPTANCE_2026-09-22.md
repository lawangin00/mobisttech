...
2. Finish missing focused negative owner/amount/reference/cross-provider/replay/refund, HTTP/admin-level provider-unavailable and operating-mode cases. Registry-level configuration-only provider-unavailable cases above are covered, but do not prove those other paths. Link individual cases to exact target HTTP/API/browser evidence rather than counting the full-suite pass as blanket parity.
