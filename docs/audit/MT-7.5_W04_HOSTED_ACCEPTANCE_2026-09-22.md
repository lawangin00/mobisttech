...
## Additional W04 negative browser gate — PASS on clean host
...
- The browser-only stub advertises JazzCash for this one simulated UI journey while all authentic external providers remain default-OFF. After an order is synthetically created, payment initiation supplies an insecure `http://` redirect. The test asserts refused navigation, explicit unsafe-redirect error, the exact created-order recovery link, no duplicate Place order action, and exactly one order-submit and one payment-initiate request. Stubbed order IDs and responses are not server-side real orders or provider receipts.
- Explicit full-scope CI request commit `c8f15d51b379afabea72134b93ec9a12444cb66b` triggered https://github.com/lawangin00/mobisttech/actions/runs/35673288185 . Request validation and clean checkout acceptance jobs both completed **success**. The clean job passed Website checkout Playwright (including the new negative test), backend regression, MySQL race/reset, website build, POS/Admin and other Website browser suites, final schema/cleanup gate, and tracked-runtime-artifact gate. W01/P02/W03 isolated family-specific steps were **skipped by this run's scope** and are not counted as fresh focused acceptance here.
