...
## Final schema residual classification ? corrected

Exact-source run `36132616755` is terminal **FAIL**. It passed request validation, Composer/Pint, backend TypeScript/build, full backend regression, MySQL race/reset, Website build/budgets/secret scan, fresh-owner browser, POS/Admin, Website checkout, Website customer/project, Website public-content and Website performance. The only failed step was **Final schema and cleanup gate**. The immediately preceding diagnostic reported exactly `CI_DISPOSABLE_RESIDUAL_TABLES=publication_versions:1`, after all browser suites had otherwise passed.

A prior checkpoint incorrectly attributed that single residual marker to `cms.presentation`; current source proves `W06PresentationE2eCleanupSeeder` contains no publication-version cleanup and no such correction was actually present. The actual bounded root is the Website media fixture lifecycle: `WebsiteCms::registerMedia()` / `deleteUnusedMedia()` bump the derived `cms.media` publication marker. `W07MediaE2eCleanupSeeder` correctly retires and removes exact-owned test media, but previously left that derived marker behind; `W06ManagedSocialMediaE2eCleanupSeeder` could also be the last exact-owned media cleanup.

The corrected cleanup removes **only** `publication_versions.domain = cms.media`, and only when the disposable testing schema has **zero** remaining `site_media_assets` and **zero** `site_media_usages`. Therefore unknown media residue is never masked. A focused regression in `WebsiteMediaLifecycleTest` creates an exact-owned W07 media asset, proves `cms.media` exists, runs the guarded cleanup, then proves media assets/usages and the marker are all absent. Result: **1/1 PASS (4 assertions)**. Scoped Pint **3 files PASS**, PHP syntax and `git diff --check` PASS.

Per owner instruction, run `36132616755` was **not restarted** and no duplicate/parallel CI request was sent. Its terminal FAIL remains authoritative historical evidence. **27/Q01 remains OPEN [E] because there is still no terminal full clean-checkout PASS on a source containing this cleanup correction.**
