...
Google Drive supports Test, Connect/Reconnect/Disconnect, Backup Now, daily/weekly/disabled schedule and retention. Backups are encrypted locally, uploaded under the new Mobisttech namespace, recorded, and locally removed. Frontends never execute rclone. The existing Windows `mobisttech-drive:` passed a live temporary read/write/delete check; the old `mobist-drive:` and archives were untouched.
