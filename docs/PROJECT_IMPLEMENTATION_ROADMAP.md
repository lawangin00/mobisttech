...
Scope: Close every valid requirement in the complete source-to-target register with fresh target evidence and independently run operational journeys. Close the addendum, consolidated-requirement and software-product-publishing traceability as well as original source parity; priorities and optional enablement do not waive implementation gates. Include Invoice/Warranty no-auto-download, Preview/Save PDF/Print/safe Email/assisted WhatsApp/history; POS Cash/Card/Mobile Wallet/Bank Transfer, split tender, cash change, destination authorization, Day Closing, payment mix/fees/refund reconciliation; Website fixed-four-channel regression; Software Product create/preview/publish/update/rollback and public route family; and legal-policy/licensing/notice acceptance.

Acceptance: Pest/PHPUnit, MySQL, builds, Playwright, POS/Website/Control, documentation and recovery gates pass; no silent feature loss or unreviewed duplicate master remains. Every approved addendum, consolidated-requirement and software-product-publishing clause has fresh backend/UI/API/performance evidence or an explicit unresolved gap. Policy coverage matches final product behavior with approved versions/effective dates and working footer/product links; Privacy claims match real data flows; software ownership and third-party notice posture are deliberate. At least the mobiST POS seed/reference set is reconciled through the reusable Software Product workflow without hard-coded one-off implementation.
