...
Every point includes applicable focused/full tests, parity evidence, secrets review, source-boundary checks and ledger updates. Routine execution progress updates the ledger only and must not edit this roadmap or regenerate its DOCX. Approved development-versus-external-activation acceptance separation is binding across all current/future stages: independently testable implementation, security, payment/data integrity and synthetic/API/browser acceptance must complete before each development-stage exit; separately registered external provider/production/owner-activation prerequisites block their applicable real-world activation or release, not unrelated development. The enforceable external prerequisite and final-audit register is `docs/HOLD_PRE_LAUNCH_REGISTER.md`; neither a HOLD nor a synthetic PASS is permission for a live transaction. Regenerate/verify the DOCX only when roadmap structure/content materially changes. MT-1.1 detailed inventory may refine this roadmap from evidence, but valid functionality must not be silently dropped and completed IDs must not be renumbered.
...
Acceptance: DEVELOPMENT ACCEPTANCE separately proves duplicate submit, price/stock changes, failed payments, synthetic verified-confirmation/refund/reconciliation and the fixed-four-channel UI/Admin configuration/backend/ownership/idempotency/retry/cancel contract end-to-end, with disabled external channels and applicable real API/browser/security/regression evidence. Authentic JazzCash/Easypaisa/card contracts, sandbox transactions, callbacks, external refunds and settlement are EXTERNAL PROVIDER ACCEPTANCE H-02 PRE-LAUNCH, not this independently executable development gate; only authentically configured providers may make sandbox claims. Website transactions classify into shared reporting/reconciliation without changing checkout choices. Measure mode-specific checkout loading with production builds; price/discount/reward tampering and usage-limit races fail safely.
...
Scope: Independently audit the complete Goal and Preferences, requirement map, all completed-point claims, Git/code/data, migration/recovery, security, integrations, CI, documentation, HOLD register and source immutability. Audit development completion separately from external-provider/pre-launch readiness; no vendor claim or live payment is inferred from a finished W04/MT-7.5/MT-7.6 development stage. Include approved requirements addendum v1.1, the unified Admin/Google requirement, the Team Member/delegated-access/session-security requirement, the complete consolidated documents/payments/legal/manual requirement and the software product publishing requirement. Inspect actual Invoice/Warranty UX and canonical delivery, Gmail attachments, truthful WhatsApp, POS payment methods/destinations/split tender/refunds/settlement, Website fixed channels, Software Product Admin/publication/version routes, Day Closing/payment mix, legal/privacy/service-pricing consistency, project ownership/third-party notices and the final User Manual.

Acceptance: Reopen every material requirement, policy, manual, payment or document-delivery mismatch; the new repository must be clean and remote-aligned. A development-only audit may report technical development acceptance with explicit PRE-LAUNCH BLOCKED status, but must NOT report `Project complete: 100%`, live payment readiness or final release sign-off while any required pre-launch/provider/production/owner/legal gate in `docs/HOLD_PRE_LAUNCH_REGISTER.md` remains unverified or unauthorized. Report `Project complete: 100%` only after the clean final audit AND all applicable mandatory external/pre-launch prerequisites are genuinely passed and authorized; Deferred required work cannot substitute for completion. Verify original completed history was preserved and every subsequent extension was independently accepted.
...
H-02 - PRE-LAUNCH EXTERNAL PROVIDER ACCEPTANCE, separately tracked from W04/MT-7.5 DEVELOPMENT ACCEPTANCE. JazzCash and Easypaisa need verified official merchant-specific contracts, selected provider mode/transport, merchant enrolment and valid sandbox credentials; card needs an owner-approved hosted/tokenized processor, official integration contract and valid sandbox credentials. Provider-specific authenticated initiation, callback/IPN signature/reference/amount/replay/failure, genuine sandbox refunds, settlement/reconciliation, security/secret rotation and truthful Admin/Customer browser behavior require exact-provider evidence and explicit owner approval before a channel is eligible for activation. Live activation additionally needs its own explicit authorization and production-safe review; until then external channels stay disabled/fail closed. W04 development can close, other roadmap families/stages can advance and 27/27 development parity can eventually be evidenced without marking H-02 PASS, live-ready or `Project complete: 100%`. `docs/HOLD_PRE_LAUNCH_REGISTER.md` defines the accountable gate and evidence; fake acceptance is prohibited.
...
HOLD/dependency evidence is explicitly reviewed in FINAL-AUDIT. Approved development-versus-activation separation also applies to Gmail, live infrastructure, external credentials and factual owner/legal decisions where local development can proceed; required internal security, data-integrity, implementation, local tests and independently executable gates are NEVER deferred to HOLD. A required capability cannot be left unimplemented and then relabeled Deferred merely to complete the project.
