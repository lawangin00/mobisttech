...
assert len(routes['previous_exact_route_dispositions'])==316 and len(routes['pending_exact_route_review'])==0
...
assert len([r for r in inv['routes'] if 'W01' in r['families']])==46
assert len(re.findall(r'^\|\s*\d+\s*\|', read('docs/audit/MT-7.5_H01_W01_IDENTITY_ROUTE_DISPOSITION_2026-09-25.md'), re.M))==46
assert 'P08 local-backup operator UI is now locally accepted' in routes['first_pending']
