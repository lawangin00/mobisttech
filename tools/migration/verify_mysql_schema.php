...
$mode = $argv[1] ?? '';
if (! in_array($mode, ['admin-google', 'warranty', 'sales', 'addendum', 'identity', 'shared', 'runtime'], true)) {
    throw new RuntimeException('Expected admin-google, warranty, sales, addendum, identity, shared or runtime verification mode.');
}
...
if (in_array($mode, ['identity', 'addendum', 'sales', 'warranty', 'admin-google'], true)) {
...
if (in_array($mode, ['addendum', 'sales', 'warranty', 'admin-google'], true)) {
...
if (in_array($mode, ['sales', 'warranty', 'admin-google'], true)) {
...
if (in_array($mode, ['warranty', 'admin-google'], true)) {
...
foreach ($actual as $table) {
    ...
    $seedRows = $mode === 'admin-google' ? ['business_profiles' => 1, 'integration_connections' => 2] : [];
...
echo json_encode(['mode' => $mode, 'result' => 'PASS', 'database' => $db->db, 'port' => (int) $db->port,
    ...
    'business_rows' => 0, 'canonical_seed_rows' => $mode === 'admin-google' ? 3 : 0], JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES)."\n";
