...
$mode = $argv[1] ?? '';
if (! in_array($mode, ['procurement', 'team-members', 'orders', 'admin-google', 'warranty', 'sales', 'addendum', 'identity', 'shared', 'runtime'], true)) {
    throw new RuntimeException('Expected procurement, team-members, orders, admin-google, warranty, sales, addendum, identity, shared or runtime verification mode.');
}
...
if (in_array($mode, ['identity', 'addendum', 'sales', 'warranty', 'admin-google', 'orders', 'team-members', 'procurement'], true)) {
...
if (in_array($mode, ['addendum', 'sales', 'warranty', 'admin-google', 'orders', 'team-members', 'procurement'], true)) {
...
if (in_array($mode, ['sales', 'warranty', 'admin-google', 'orders', 'team-members', 'procurement'], true)) {
...
if (in_array($mode, ['warranty', 'admin-google', 'orders', 'team-members', 'procurement'], true)) {
...
if (in_array($mode, ['admin-google', 'orders', 'team-members', 'procurement'], true)) {
...
if (in_array($mode, ['orders', 'team-members', 'procurement'], true)) {
...
if (in_array($mode, ['team-members', 'procurement'], true)) {
...
foreach ($actual as $table) {
    ...
    $seedRows = in_array($mode, ['admin-google', 'orders', 'team-members', 'procurement'], true) ? ['business_profiles' => 1, 'integration_connections' => 2] : [];
    if (in_array($mode, ['team-members', 'procurement'], true)) {
...
echo json_encode(['mode' => $mode, 'result' => 'PASS', 'database' => $db->db, 'port' => (int) $db->port,
    ...
    'business_rows' => 0, 'canonical_seed_rows' => in_array($mode, ['team-members', 'procurement'], true)
