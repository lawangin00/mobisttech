...
export function CustomerAccountPanel() {
  ...
  return <div className="space-y-6">
    ...
    <section><h2 className="text-xl font-bold">Orders</h2><div className="mt-3 space-y-3">{!snapshotReady ? <p className="text-slate-600">Loading orders…</p> : orders.length === 0 ? <p className="text-slate-600">No orders yet.</p> : orders.map((order) => <article key={order.id} className="rounded-2xl border bg-white p-4"><Link href={"/account/orders/" + order.id} prefetch={false} className="font-semibold underline-offset-2 hover:underline">{order.number}</Link><p className="text-sm text-slate-600">{order.status} · {order.payment_status} · {order.currency} {order.total}</p></article>)}</div></section>
