...
export function CustomerOrderDetail({ orderId }: { orderId: string }) {
  ...
  return <div className="min-h-[620px] space-y-6">
    ...
    <div className="flex flex-wrap gap-3">
      {order.status === "pending" && order.payment_status === "unpaid" && latest && latest.gateway !== "cod" && latest.status === "pending" && <button disabled={busy} onClick={() => void initiate(latest.public_id).catch((error) => setMessage(error instanceof Error ? error.message : "Unable to continue payment."))} className="rounded-xl bg-slate-950 px-4 py-2 text-white">Continue payment</button>}
