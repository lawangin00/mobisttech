...
if (backend !== "http://127.0.0.1:18080") throw new Error("Website media origin must be isolated Laravel target.");
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ slug: string; media: string }> }) {
  ...
  const response = await fetch(`${backend}/api/v1/software/${encodeURIComponent(slug)}/media/${media}`, {
