...
final class CreateGoogleDriveBackup implements ShouldQueue
{
    ...
    public function handle(BackupArchive $archive, RcloneGateway $rclone): void
    {
        ...
        $remote = config('backups.namespace').app()->environment().'/'.now()->format('Y/m/d').'/'.$file['filename'];
        try {
            $rclone->copyTo($file['path'], $remote, $managed);
            DB::table('backup_records')->where('id', $this->recordId)->update([
                'status' => 'completed', 'remote_status' => 'uploaded', 'remote_path' => $remote,
                'completed_at' => now(), 'updated_at' => now(),
            ]);
            DB::table('integration_connections')->where('id', $record->integration_connection_id)->update([
            ...
            if ($cutoff) {
                foreach (DB::table('backup_records')->where('integration_connection_id', $record->integration_connection_id)
                    ->where('status', 'completed')->where('completed_at', '<', $cutoff)->whereNotNull('remote_path')->get() as $expired) {
    ...
    {
        DB::table('backup_records')->where('id', $this->recordId)->update([
            'status' => 'failed', 'remote_status' => 'failed', 'error_message' => 'Google Drive backup failed.',
            'completed_at' => now(), 'updated_at' => now(),
        ]);
    }
}
