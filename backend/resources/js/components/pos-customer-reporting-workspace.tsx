...
type Report = {
    ...
    categories?: Array<{ category: string; label: string; units_sold: number; net_sales: string; profit: string; products: number; stock_units: number; inventory_cost: string; claims: number }>;
    activity: Record<string, number>;
};
type ListColumn={id:string;label:string;visible:boolean;order:number};
type Paging = {page:number;pages:number;total:number;per_page:number;q:string;category:string;options:string[];auto_focus_search:boolean;remember_search:boolean;density:'comfortable'|'compact';sort:string;filter:string;columns:ListColumn[]};
type ReportSection={key:string;label:string;visible:boolean;order:number};
type Data = { area: Area; outlet: { id: string; name: string }; can_send_documents: boolean; invoice_default_format:'a4'|'thermal80'|null; invoices: Invoice[]; customers: Customer[]; claims: ClaimRow[]; sale_candidates: SaleCandidate[]; report: Report | null; report_presentation:{sections:ReportSection[]}|null; pagination: Paging | null; warranty_intake_category: string | null };
...
export default function PosCustomerReportingWorkspace({ area, memoryScope }: { area: Area; memoryScope: string }) {
    ...
    const load = async (next = filter) => {
        ...
        const enabled = result.pagination?.remember_search === true;
        setTabConsent(tabSearchOptedIn(memoryScope,enabled));
        const remembered = result.pagination ? tabSearchCategory(memoryScope,enabled,area,result.pagination.options) : null;
        if (next.q === '' && next.category === '' && next.page === 1 && remembered && remembered !== result.pagination?.category) return load({...next,category:remembered});
    ...
    return <div data-testid={'mt43-' + area} data-density={density} className={'mt-6 grid min-w-0 '+(density==='compact'?'gap-3':'gap-5')}>
        ...
        {area === 'claims' && <Claims data={data} busy={busy} run={run} area={area} memoryScope={memoryScope} />}
...
function Invoices({ data, busy, run }: { data: Data; busy: boolean; run: (task: () => Promise<void>, reload?: boolean) => Promise<void> }) {
    ...
    const column=(id:string)=>data.pagination?.columns.find(item=>item.id===id)??{id,label:id,visible:true,order:999};
    return <>
        <section className="min-w-0 rounded-2xl border bg-white p-5"><h3 className="font-semibold">Customers & invoice history</h3>
            ...
            <div><select value={selected?.id ?? ''} onChange={(e) => setInvoiceId(e.target.value)} className="w-full min-w-0 rounded border p-2 text-sm"><option value="">Invoice</option>{data.invoices.map((row) => <option key={row.id} value={row.id}>{row.number} · {row.customer_name ?? 'Walk-in'} · PKR {row.final_bill}</option>)}</select>{selected && <div className="mt-2 flex flex-col rounded bg-slate-50 p-3 text-sm"><p hidden={!column('total').visible} style={{order:column('total').order}}>PKR {selected.final_bill}</p><p hidden={!column('customer').visible} style={{order:column('customer').order}}>{selected.customer_name ?? 'Walk-in'}</p><p hidden={!column('contact').visible} style={{order:column('contact').order}}>{selected.customer_phone ?? '—'} · {selected.customer_email ?? '—'}</p><p hidden={!column('date').visible} style={{order:column('date').order}}>{selected.created_at}</p><p hidden={!column('salesperson').visible} style={{order:column('salesperson').order}}>{selected.salesperson_name ?? '—'}</p></div>}</div></div>
        </section>
        {selected && <DocumentActions key={selected.id} type="invoice" documentId={selected.id} canSend={data.can_send_documents} busy={busy} run={run} defaultFormat={data.invoice_default_format ?? 'a4'} />}
    </>;
}
...
function WarrantyHistory({ data, busy, run }: { data: Data; busy: boolean; run: (task: () => Promise<void>, reload?: boolean) => Promise<void> }) {
    ...
    const column=(id:string)=>data.pagination?.columns.find(item=>item.id===id)??{id,label:id,visible:true,order:999};
    return <>
        <section className="min-w-0 rounded-2xl border bg-white p-5"><h3 className="font-semibold">Warranty Claim Receipts</h3><p className="mt-1 text-xs text-slate-500">Historical warranty receipts are outlet-scoped. Claim intake/lifecycle remains in Claims.</p><select value={selected?.id ?? ''} onChange={(e) => setSelectedId(e.target.value)} className="mt-3 w-full min-w-0 rounded border p-2 text-sm"><option value="">Warranty claim</option>{data.claims.map((row) => <option key={row.id} value={row.id}>{row.number} · {row.product_name} · {row.status}</option>)}</select>{selected && <div className="mt-3 flex flex-col rounded bg-slate-50 p-3 text-sm"><p style={{order:column('claim').order}}>{selected.number}</p><p hidden={!column('invoice_customer').visible} style={{order:column('invoice_customer').order}}>{selected.invoice_number} · {selected.customer_name ?? '—'}</p><p hidden={!column('device').visible} style={{order:column('device').order}}>{selected.product_name}</p><p hidden={!column('received').visible} style={{order:column('received').order}}>received {selected.received_at}</p><p hidden={!column('status').visible} style={{order:column('status').order}}>{selected.status}</p></div>}</section>
...
function Claims({ data, busy, run, area: _area, memoryScope }: { data: Data; busy: boolean; run: (task: () => Promise<void>, reload?: boolean) => Promise<void>; area: 'claims'; memoryScope: string }) {
    const [selectedId, setSelectedId] = useState(data.claims[0]?.id ?? '');
    const [detail, setDetail] = useState<ClaimDetail | null>(null);
    const [saleId, setSaleId] = useState('');
    const [intakeTerm, setIntakeTerm] = useState('');
    const intakeOptions = ['all','invoice_id','customer_name','customer_cnic','contact_number','product','imei'];
    const [intakeCategory, setIntakeCategory] = useState(tabSearchCategory(memoryScope,data.pagination?.remember_search===true,'claims-intake',intakeOptions) ?? data.warranty_intake_category ?? 'all');
    ...
    return <>
        ...
        <section className="min-w-0 rounded-2xl border bg-white p-5"><h3 className="font-semibold">Historical claims</h3><select value={data.claims.some((row)=>row.id===selectedId)?selectedId:''} onChange={(e) => void loadClaim(e.target.value)} className="mt-3 w-full min-w-0 rounded border p-2 text-sm"><option value="">Claim</option>{data.claims.map((row) => <option key={row.id} value={row.id}>{row.number} · {row.product_name} · {row.status}</option>)}</select>
            {detail && <div className="mt-3 grid gap-3"><div className="flex flex-col rounded bg-slate-50 p-3 text-sm"><p style={{order:column('claim').order}}>{String(detail.claim_number)}</p><p hidden={!column('status').visible} style={{order:column('status').order}}><strong>{String(detail.status)}</strong></p><p hidden={!column('invoice_customer').visible} style={{order:column('invoice_customer').order}}>{String(detail.invoice_number)} · {String(detail.customer_name ?? '')}</p><p hidden={!column('device').visible} style={{order:column('device').order}}>{String(detail.product_name ?? '')}</p></div>
...
function Reports({ data, busy, run, from, to, setFrom, setTo, reload }: { data: Data; busy: boolean; run: (task: () => Promise<void>, reload?: boolean) => Promise<void>; from: string; to: string; setFrom: (v: string) => void; setTo: (v: string) => void; reload: () => Promise<void> }) {
    ...
    const configured=data.report_presentation?.sections??[];
    const section=(key:string)=>configured.find(item=>item.key===key)??{key,label:key,visible:true,order:999};
    ...
    return <section className="min-w-0 rounded-2xl border bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">Outlet dashboard & reports</h3><p className="text-xs text-slate-500">Invoice totals are counted once; Payment Mix keeps POS and Website channels distinct.</p></div><div className="flex flex-wrap gap-2"><input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="rounded border p-2 text-sm" /><input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="rounded border p-2 text-sm" /><button disabled={busy} onClick={() => void run(reload, false)} className="rounded border px-3 py-2 text-sm">Apply</button><button disabled={busy} onClick={() => void run(exportCsv, false)} className="rounded border px-3 py-2 text-sm">Export CSV</button></div></div>
        <div className="flex flex-col"><div hidden={!section('sales').visible} style={{order:section('sales').order}} className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{Object.entries(report.sales).filter(([,v]) => typeof v !== 'string' || !String(v).startsWith('Invoice totals')).map(([key,value]) => <div key={key} className="rounded bg-slate-50 p-3 text-sm"><span className="block text-xs text-slate-500">{key.replaceAll('_',' ')}</span><strong>{String(value)}</strong></div>)}</div>
        <div hidden={!section('payments').visible} style={{order:section('payments').order}} className="mt-5 rounded-xl border p-4"><div className="flex flex-wrap items-center justify-between gap-2"><h4 className="font-semibold">Payment Mix</h4><button onClick={() => setShowDestinations((old) => !old)} className="rounded border px-2 py-1 text-xs">{showDestinations ? 'Hide destination detail' : 'Destination drill-down'}</button></div><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><div>POS: <strong>PKR {report.payments.pos_tender_total}</strong></div><div>Website: <strong>PKR {report.payments.website_payment_total}</strong></div><div>Fees: <strong>PKR {report.payments.provider_fees}</strong></div><div>Settlement variance: <strong>PKR {report.payments.settlement_variance}</strong></div>{Object.entries(report.payments.method_breakdown).map(([method,value]) => <div key={method}>{method}: <strong>PKR {value}</strong></div>)}{Object.entries(report.payments.website_gateway_breakdown).map(([gateway,value]) => <div key={'w-'+gateway}>Website {gateway}: <strong>PKR {value}</strong></div>)}</div>{showDestinations && <div className="mt-3 grid gap-2">{report.payments.destination_breakdown.map((row,index) => <div key={index} className="rounded bg-slate-50 p-2 text-sm">{row.destination} · {row.method} · PKR {row.amount}</div>)}</div>}</div>
        <div hidden={!section('categories').visible} style={{order:section('categories').order}} className="mt-5 overflow-x-auto rounded-xl border"><table className="w-full min-w-[720px] text-left text-sm"><caption className="p-4 text-left font-semibold">Category performance and inventory</caption><thead className="bg-slate-50"><tr><th className="p-2">Category</th><th className="p-2">Units sold</th><th className="p-2">Net sales</th><th className="p-2">Profit</th><th className="p-2">Products</th><th className="p-2">Stock</th><th className="p-2">Inventory cost</th><th className="p-2">Claims</th></tr></thead><tbody>{(report.categories ?? []).map(row=><tr key={row.category} className="border-t"><td className="p-2">{row.label}</td><td className="p-2">{row.units_sold}</td><td className="p-2">PKR {row.net_sales}</td><td className="p-2">PKR {row.profit}</td><td className="p-2">{row.products}</td><td className="p-2">{row.stock_units}</td><td className="p-2">PKR {row.inventory_cost}</td><td className="p-2">{row.claims}</td></tr>)}</tbody></table></div>
        <div hidden={!section('activity').visible} style={{order:section('activity').order}} className="mt-5 grid gap-2 sm:grid-cols-3">{Object.entries(report.activity).map(([key,value]) => <div key={key} className="rounded border p-3 text-sm"><span className="text-slate-500">{key.replaceAll('_',' ')}</span><strong className="ml-2">{value}</strong></div>)}</div></div>
    </section>;
}
