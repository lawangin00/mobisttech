...
export default function PosMasterDataWorkspace(){
    ...
    const [editId,setEditId]=useState<number|null>(null); const [editLabel,setEditLabel]=useState('');
    const [editOrder,setEditOrder]=useState('0'); const [editHex,setEditHex]=useState('#111111');
    const load=async()=>{
    ...
    const change=async(action:string,id?:number)=>{
        ...
        try{
            ...
            if(action==='update'&&id===editId){if(!numeric)input.label=editLabel.trim();input.sort_order=Number(editOrder);
                if(list==='unit_color')input.hex=editHex;}
            ...
            await load();setLabel('');if(action==='update')setEditId(null);setMessage('Option '+action+' successful.');
    ...
    return <section data-testid="master-data-workspace" className="mt-5 grid gap-4 rounded-xl border bg-white p-4">
        ...
        <label className="grid gap-1 text-sm">Option list<select data-testid="master-list" value={list} onChange={e=>{setList(e.target.value);setMessage('');setEditId(null);}} className="rounded border p-2">{Object.entries(catalogue?.lists??{}).map(([key,name])=><option key={key} value={key}>{name}</option>)}</select></label>
        ...
        <div className="grid gap-2">{options.map(option=><div key={option.id} data-testid={'master-row-'+option.code} className="flex flex-wrap items-center justify-between gap-2 rounded border p-3 text-sm"><div><strong>{option.label}</strong><p className="text-xs text-slate-500">{option.code} · {option.usages_count} recorded uses · {option.archived_at?'Archived':option.is_active?'Active':'Inactive'}</p></div>
            <div className="flex flex-wrap gap-1"><button disabled={busy} className="rounded border px-2 py-1" onClick={()=>{setEditId(option.id);setEditLabel(option.label);setEditOrder(String(option.sort_order));setEditHex(String(option.metadata?.hex??'#111111'));}}>Edit</button>{!option.archived_at&&<button disabled={busy} className="rounded border px-2 py-1" onClick={()=>void change(option.is_active?'deactivate':'activate',option.id)}>{option.is_active?'Deactivate':'Activate'}</button>}
            ...
            </div>{editId===option.id&&<form data-testid="master-editor" className="grid w-full gap-2 rounded border p-2 sm:grid-cols-2" onSubmit={event=>{event.preventDefault();void change('update',option.id);}}>
                {!numeric&&<input data-testid="master-edit-label" aria-label="Edit option label" required maxLength={160} value={editLabel} onChange={e=>setEditLabel(e.target.value)} className="rounded border p-2"/>}
                <input data-testid="master-edit-order" aria-label="Sort order" required type="number" min="0" max="100000" step="1" value={editOrder} onChange={e=>setEditOrder(e.target.value)} className="rounded border p-2"/>
                {list==='unit_color'&&<input data-testid="master-edit-hex" aria-label="Edit color hex" required pattern="#[0-9a-fA-F]{6}" value={editHex} onChange={e=>setEditHex(e.target.value)} className="rounded border p-2"/>}
                <button data-testid="master-edit-save" disabled={busy||(!numeric&&!editLabel.trim())} className="rounded bg-slate-950 p-2 text-white">Save presentation</button>
                <button type="button" className="rounded border p-2" onClick={()=>setEditId(null)}>Cancel</button>
            </form>}</div>)}</div>
    </section>;
}
