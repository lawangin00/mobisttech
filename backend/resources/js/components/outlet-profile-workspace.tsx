...
export default function OutletProfileWorkspace({outletId}:{outletId:string}) {
    const [profile,setProfile] = useState<OutletProfile|null>(null);
    const [form,setForm] = useState<Form|null>(null);
    const [busy,setBusy] = useState(false);
    const [message,setMessage] = useState('');
    async function load() {
        const response = await fetch('/internal/admin/pos/outlet-profile',{headers:{Accept:'application/json'},credentials:'same-origin'});
    ...
    async function save() {
        ...
        try {
            ...
            const response=await fetch('/internal/admin/pos/outlet-profile',{
            ...
            await load();setMessage('Outlet profile saved.');
        } catch(error) {setMessage(error instanceof Error?error.message:'Save failed.');}
        finally {setBusy(false);}
    }
    return <div data-testid="outlet-profile-editor" className="mt-6 rounded-xl border bg-slate-50 p-4">
        <p className="text-sm text-slate-600">Only your selected and assigned outlet can be edited. Its original three-character code and historical records cannot be changed here.</p>
        {profile&&form?<><p className="mt-3 text-xs font-semibold">Outlet code: <span data-testid="outlet-profile-code">{profile.outlet_code}</span></p>
            ...
            <button data-testid="outlet-profile-save" disabled={busy||!form.name.trim()} onClick={()=>void save()} className="mt-4 rounded bg-slate-950 px-4 py-2 text-sm text-white disabled:opacity-50">Save outlet profile</button></>:<p className="mt-2 text-sm">Loading the selected outlet...</p>}
