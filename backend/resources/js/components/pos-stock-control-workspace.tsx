...
export default function PosStockControlWorkspace() {
    ...
    return <section data-testid="stock-control" className="min-w-0 rounded-2xl border border-slate-300 bg-slate-50 p-4">
...
function Procurement({ data, busy, run }: { data: Data; busy: boolean; run: (task: () => Promise<void>) => Promise<void> }) {
    ...
    return <div className="grid min-w-0 gap-4 rounded-xl border bg-white p-4">
        <h4 className="font-semibold">Suppliers & purchase orders</h4>
        <div className="grid grid-cols-2 gap-2"><input value={code} onChange={(e) => setCode(e.target.value)} placeholder="Supplier code" className="w-full min-w-0 rounded border p-2 text-sm" /><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Supplier name" className="w-full min-w-0 rounded border p-2 text-sm" /></div>
        ...
        <div className="grid gap-2 sm:grid-cols-2"><select value={supplier} onChange={(e) => setSupplier(e.target.value)} className="w-full min-w-0 rounded border p-2 text-sm"><option value="">Supplier</option>{data.suppliers.filter((s) => s.is_active).map((s) => <option key={s.public_id} value={s.public_id}>{s.supplier_code} · {s.name}</option>)}</select><select value={product} onChange={(e) => setProduct(e.target.value)} className="w-full min-w-0 rounded border p-2 text-sm"><option value="">Product</option>{data.products.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select><input value={qty} onChange={(e) => setQty(e.target.value)} placeholder="Order qty" className="w-full min-w-0 rounded border p-2 text-sm" /><input value={cost} onChange={(e) => setCost(e.target.value)} placeholder="Unit / landed cost" className="w-full min-w-0 rounded border p-2 text-sm" /></div>
        ...
        <div className="grid gap-2 sm:grid-cols-3"><select value={orderId} onChange={(e) => { setOrderId(e.target.value); setLineId(''); }} className="w-full min-w-0 rounded border p-2 text-sm"><option value="">Open PO</option>{data.orders.filter((o) => ['ordered','partially_received'].includes(o.status)).map((o) => <option key={o.id} value={o.id}>{o.number} · {o.supplier_name}</option>)}</select><select value={lineId} onChange={(e) => setLineId(e.target.value)} className="w-full min-w-0 rounded border p-2 text-sm"><option value="">PO line</option>{order?.lines.map((l) => <option key={l.line_id} value={l.line_id}>{l.name} · {l.received_quantity}/{l.ordered_quantity}</option>)}</select><input value={receiveQty} onChange={(e) => setReceiveQty(e.target.value)} placeholder="Receive qty" className="w-full min-w-0 rounded border p-2 text-sm" /></div>
        ...
        <div className="grid gap-2 sm:grid-cols-3"><select value={product} onChange={(e) => setProduct(e.target.value)} className="w-full min-w-0 rounded border p-2 text-sm"><option value="">Reorder product</option>{data.products.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select><input value={threshold} onChange={(e) => setThreshold(e.target.value)} placeholder="Threshold" className="w-full min-w-0 rounded border p-2 text-sm" /><input value={target} onChange={(e) => setTarget(e.target.value)} placeholder="Target stock" className="w-full min-w-0 rounded border p-2 text-sm" /></div>
...
function Stocktake({ data, busy, run }: { data: Data; busy: boolean; run: (task: () => Promise<void>) => Promise<void> }) {
    ...
    return <div className="grid min-w-0 gap-3 rounded-xl border bg-white p-4">
        <h4 className="font-semibold">Stocktake / recount / approval</h4>
        <div className="grid grid-cols-2 gap-2"><select value={kind} onChange={(e) => setKind(e.target.value)} className="w-full min-w-0 rounded border p-2 text-sm"><option value="cycle">Cycle count</option><option value="full">Full count</option></select><select disabled={kind === 'full'} value={product} onChange={(e) => setProduct(e.target.value)} className="w-full min-w-0 rounded border p-2 text-sm"><option value="">Cycle product</option>{data.products.map((x) => <option key={x.id} value={x.id}>{x.name}</option>)}</select></div>
        ...
        <select value={id} onChange={(e) => void loadSession(e.target.value)} className="w-full min-w-0 rounded border p-2 text-sm"><option value="">Existing stocktake</option>{data.stocktakes.map((s) => <option key={s.public_id} value={s.public_id}>{s.session_number} · {s.status}</option>)}</select>
        {line && session && <div className="grid gap-2 rounded bg-slate-50 p-3 text-sm"><p><strong>{line.product_name}</strong> · baseline {line.baseline_quantity} · iteration {line.iteration}</p>{line.tracked_serialized ? <input value={units} onChange={(e) => setUnits(e.target.value)} placeholder="Scan/paste unit IDs, comma-separated" className="w-full min-w-0 rounded border bg-white p-2" /> : <input value={count} onChange={(e) => setCount(e.target.value)} placeholder="Counted quantity" className="w-full min-w-0 rounded border bg-white p-2" />}<select value={reason} onChange={(e) => setReason(e.target.value)} className="w-full min-w-0 rounded border bg-white p-2"><option value="count_error">Count error</option><option value="damage">Damage</option><option value="loss">Loss</option><option value="found_stock">Found stock</option><option value="misplacement">Misplacement</option><option value="other">Other</option></select><button disabled={busy || session.status !== 'counting'} onClick={() => void run(async () => { await api('/internal/admin/pos/stock-control/stocktakes/' + id + '/lines/' + line.line_id, { method: 'POST', body: JSON.stringify({ line_version: line.line_version, counted_quantity: line.tracked_serialized ? null : Number(count), unit_ids: line.tracked_serialized ? units.split(',').map((x) => x.trim()).filter(Boolean) : null, reason_code: reason }) }); await loadSession(id); })} className="rounded bg-slate-950 px-3 py-2 font-semibold text-white">Submit count</button></div>}
...
function Bulk({ busy, run }: { busy: boolean; run: (task: () => Promise<void>) => Promise<void> }) {
    ...
    return <div className="grid min-w-0 gap-3 rounded-xl border bg-white p-4">
        <h4 className="font-semibold">Validated bulk operations</h4><div className="grid grid-cols-2 gap-2"><select value={dataset} onChange={(e) => setDataset(e.target.value)} className="w-full min-w-0 rounded border p-2 text-sm"><option value="catalogue">Catalogue</option><option value="price">Price</option><option value="inventory">Inventory</option></select><select value={recovery} onChange={(e) => setRecovery(e.target.value)} className="w-full min-w-0 rounded border p-2 text-sm"><option value="whole_batch">Whole batch</option><option value="row">Row recovery</option></select></div><textarea value={document} onChange={(e) => setDocument(e.target.value)} placeholder="Paste CSV including headers. Preview is mandatory before import." rows={7} className="w-full min-w-0 rounded border p-2 font-mono text-xs" /><div className="flex flex-wrap gap-2"><button data-testid="bulk-preview" disabled={busy || !document} onClick={() => void run(async () => setPreview(await api('/internal/admin/pos/stock-control/bulk/preview', { method: 'POST', body: JSON.stringify({ dataset, format: 'csv', document }) })))} className="rounded border px-3 py-2 text-sm">Preview</button><button disabled={busy || !preview} onClick={() => void run(async () => setPreview(await api('/internal/admin/pos/stock-control/bulk/import', { method: 'POST', body: JSON.stringify({ dataset, format: 'csv', document, recovery }) })))} className="rounded bg-slate-950 px-3 py-2 text-sm font-semibold text-white disabled:opacity-40">Import validated rows</button><button disabled={busy} onClick={() => void run(exportFile)} className="rounded border px-3 py-2 text-sm">Export CSV</button></div>{preview && <pre data-testid="bulk-result" className="max-h-56 max-w-full overflow-auto rounded bg-slate-950 p-3 text-xs text-white">{JSON.stringify(preview, null, 2)}</pre>}
    </div>;
}
