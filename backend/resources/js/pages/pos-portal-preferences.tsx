...
type ColumnDefinition={label:string;order:number;can_hide:boolean;can_reorder:boolean};
type ColumnValue={visible:boolean;order:number};
type Values=Record<string,unknown>&{navigation:Record<string,NavigationOverride>;columns:Record<string,Record<string,ColumnValue>>};
type Props={values:Values;options:Record<string,string[]>;navigation:NavigationItem[];column_catalogue:Record<string,Record<string,ColumnDefinition>>};
const labels:Record<string,string>={invoice_page_length:'Invoices per page',inventory_page_length:'Inventory rows per page',claims_page_length:'Warranty jobs per page',invoice_search_category:'Default invoice search category',inventory_search_category:'Default inventory search category',warranty_search_category:'Default warranty search category',claims_search_category:'Default warranty jobs search category',invoice_density:'Invoice history density',inventory_density:'Inventory density',claims_density:'Warranty and claims density',inventory_sort:'Inventory default sort',inventory_filter:'Inventory default filter',invoice_sort:'Invoice default sort',invoice_filter:'Invoice default filter',claims_sort:'Warranty and claims default sort',claims_filter:'Warranty and claims default filter',auto_focus_search:'Focus search automatically',remember_search:'Allow per-operator opt-in search category memory (current tab only; never search text)'};
export default function PosPortalPreferences({values,options,navigation,column_catalogue}:Props){
    ...
    return <><Head title="POS portal preferences"/><main className="min-h-screen bg-slate-100 p-5 text-slate-950 sm:p-8"><div className="mx-auto max-w-3xl space-y-5">
        ...
        <form onSubmit={submit} className="grid gap-4 rounded-xl border bg-white p-5 sm:grid-cols-2" data-testid="portal-preferences-form">
            ...
            <section className="grid gap-4 border-t pt-4 sm:col-span-2" data-testid="column-presentation"><div><h2 className="font-semibold">Responsive list field presentation</h2><p className="text-xs text-slate-500">Visibility and order affect display only. Required identifiers/actions and backend permissions cannot be changed.</p></div>{Object.entries(column_catalogue).map(([area,definitions])=><div key={area} className="grid gap-2"><h3 className="text-sm font-semibold capitalize">{area}</h3>{Object.entries(definitions).map(([id,definition])=>{const current=form.columns[area][id];return <div key={id} className="grid gap-2 rounded border p-3 sm:grid-cols-[1fr_auto_7rem]"><span className="text-sm">{definition.label}</span><label className="flex items-center gap-2 text-xs"><input disabled={!definition.can_hide||busy} type="checkbox" checked={current.visible} onChange={event=>{setSaved(false);setForm(previous=>({...previous,columns:{...previous.columns,[area]:{...previous.columns[area],[id]:{...current,visible:event.target.checked}}}}));}}/>Visible</label><label className="text-xs">Order<input disabled={!definition.can_reorder||busy} type="number" min="10" max="899" value={current.order} onChange={event=>{setSaved(false);setForm(previous=>({...previous,columns:{...previous.columns,[area]:{...previous.columns[area],[id]:{...current,order:Number(event.target.value)}}}}));}} className="mt-1 w-full rounded border p-2 text-sm"/></label></div>;})}</div>)}</section>
