...
export default function ResetAdministration({identity}:{identity:{name:string;job_title:string|null}}){
    ...
    useEffect(()=>{if(!current)return;setDomains(current.factory_scope_locked?[...current.available_domains]:[]);setPreview(null);setConfirmation('');},[level,data]);
    ...
    const previewReset=()=>run(async()=>{const value=await api<Preview>('/internal/admin/reset-administration/preview',{method:'POST',body:JSON.stringify({level,domains})});setPreview(value);setConfirmation('');},true);
