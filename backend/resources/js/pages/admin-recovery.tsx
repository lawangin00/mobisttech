...
export default function AdminRecovery({mode,offline_owner_recovery_available=false}:{mode:'request'|'reset';offline_owner_recovery_available?:boolean}) {
    const query = new URLSearchParams(window.location.search);
    const [email,setEmail]=useState(query.get('email')??'');
    const [token,setToken]=useState(query.get('token')??'');
    const [offline,setOffline]=useState(offline_owner_recovery_available&&query.get('offline')==='1');
    ...
    async function submit(event:FormEvent<HTMLFormElement>){
        ...
        try {
            if(mode==='reset') {
                ...
                if(offline) await adminAuthRequest('/internal/admin/auth/offline-owner-recovery/redeem','POST',
                    {email,code,password:secret,password_confirmation:confirm});
                else await adminAuthRequest('/internal/admin/auth/reset-password','POST',{email,token,password:secret,password_confirmation:confirm});
                setToken('');setCode('');setSecret('');setConfirm('');setCompleted(true);
    ...
    return <><Head title="Team Member account recovery"/><main className="min-h-screen bg-slate-100 p-5 text-slate-950 sm:p-10"><section className="mx-auto max-w-lg space-y-4 rounded-2xl border bg-white p-6">
        ...
        <p className="text-sm text-slate-600">{offline?'Enter one unused offline owner recovery code. Other Team Members cannot use this method.':mode==='request'?'Recovery email is available only when the approved delivery integration is enabled.':'Use the single-use link sent to your registered email.'}</p>
        {offline_owner_recovery_available&&mode==='request'&&<Link href="/internal/admin/reset-password?offline=1" className="inline-block text-sm underline">Use an offline owner recovery code</Link>}
        {offline_owner_recovery_available&&mode==='reset'&&!completed&&<button type="button" onClick={()=>{setOffline(!offline);setMessage('');}} className="text-sm underline">{offline?'Use an email recovery link':'Use an offline owner recovery code'}</button>}
        {!completed&&<form onSubmit={submit} className="space-y-3">
            ...
            {mode==='reset'&&offline&&<label className="block text-sm">One-time offline owner recovery code<input data-testid="offline-owner-recovery-code" type="password" autoComplete="off" required value={code} onChange={e=>setCode(e.target.value)} className="mt-1 w-full rounded border p-2"/></label>}
            {mode==='reset'&&<>
            ...
            <button data-testid="recovery-submit" type="submit" disabled={busy||(mode==='reset'&&!(offline?code:token))} className="rounded bg-slate-950 px-4 py-2 text-sm text-white disabled:opacity-50">{mode==='request'?'Request recovery link':'Reset password'}</button>
        </form>}
        {mode==='reset'&&!token&&!offline&&!completed&&<p role="alert" className="text-sm">A valid recovery link is required.</p>}
