...
/** Parallel real HTTP requests to two owned localhost PHP servers on disposable MySQL. */
final class W04CodNetworkRaceTest extends TestCase
{
    ...
    public function test_two_independent_network_servers_serialize_authenticated_cod_requests(): void
    {
        ...
        try {
            ...
            unset($job);
            $publishes = $this->networkRace($jobs);
    ...
    /** Own exactly two test-only PHP web servers; refuse any occupied port. */
    private function startServers(): array
    {
        $servers = [];
        try {
            foreach ([18171, 18172] as $port) {
                $socket = @stream_socket_server('tcp://127.0.0.1:'.$port, $code, $message);
                $this->assertNotFalse($socket, 'Network test port is already occupied: '.$port);
                fclose($socket);
                $server = new Process([PHP_BINARY, '-S', '127.0.0.1:'.$port,
                    base_path('vendor/laravel/framework/src/Illuminate/Foundation/resources/server.php')],
                    base_path('public'), ['APP_ENV' => 'testing', 'DB_CONNECTION' => 'mysql',
                ...
                $this->assertNotFalse($ready ?: false, 'Owned HTTP test server did not start: '.$port);
            }
    ...
    private function networkRace(array $jobs): array
    {
        ...
        try {
            try {
                foreach ($jobs as $job) {
                    $path = sys_get_temp_dir().'/w04-network-'.Str::uuid().'.json';
                    $job['marker'] = $path.'.ready';
                    file_put_contents($path, json_encode($job, JSON_THROW_ON_ERROR));
                    $paths[] = $path;
                    $worker = new Process([PHP_BINARY, base_path('tests/Support/W04CodNetworkWorker.php'),
                        $path, 'http://127.0.0.1:'.(18171 + count($workers))], base_path(), ['APP_ENV' => 'testing', 'DB_CONNECTION' => 'mysql',
                            'DB_DATABASE' => 'mobisttech_test', 'DB_URL' => '', 'SESSION_DRIVER' => 'database']);
